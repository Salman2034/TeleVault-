...
export const FileBrowser: React.FC<FileBrowserProps> = ({
  ...
  return (
    <div
      ...
      onDrop={handleDrop}
      className="relative min-h-[500px] bg-white rounded-2xl border border-slate-200/90 shadow-xs flex flex-col"
    >
      {/* Hidden file input */}
      <input
        ref={fileInputRef}
      ...
      {/* Drag & Drop Overlay */}
      {isDragOver && (
        <div className="absolute inset-0 z-30 bg-sky-500/10 backdrop-blur-xs border-2 border-dashed border-sky-500 rounded-2xl flex flex-col items-center justify-center p-6 text-sky-800 animate-in fade-in duration-100">
          <div className="w-16 h-16 rounded-2xl bg-white shadow-lg flex items-center justify-center text-sky-600 mb-3">
            <Upload className="w-8 h-8 animate-bounce" />
          </div>
          <h3 className="text-base font-bold text-slate-900">Drop files to host on Telegram</h3>
          <p className="text-xs text-slate-600 mt-1">Files will be streamed straight to Telegram CDN storage</p>
        </div>
      )}

      {/* Top Toolbar */}
      <div className="px-5 py-4 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-slate-50/40 rounded-t-2xl">
        {/* Breadcrumb Navigation */}
        <div className="flex items-center gap-1.5 text-xs text-slate-600 overflow-x-auto scrollbar-none py-0.5">
          <button
            id="breadcrumb-root-btn"
            onClick={() => onNavigateFolder(null)}
            className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg transition-colors ${
              currentFolderId === null
                ? 'font-semibold text-slate-900 bg-white shadow-xs border border-slate-200/80'
        ...
        <div className="flex items-center gap-2 self-end sm:self-center shrink-0">
          ...
          {/* Sort Selector */}
          <div className="flex items-center gap-1 bg-white border border-slate-200 rounded-xl px-2.5 py-1 text-xs text-slate-600">
            ...
            <select
              ...
              className="bg-transparent border-none text-xs text-slate-700 focus:outline-none cursor-pointer pr-1 font-medium"
            >
              <option value="updatedAt-desc">Date Modified (Newest)</option>
              <option value="updatedAt-asc">Date Modified (Oldest)</option>
      ...
      <div className="flex-1 p-5 overflow-y-auto">
        ...
        {hasSelected && (
          <div
            ...
            className="sticky top-0 z-30 mb-5 bg-slate-900 text-white rounded-2xl p-3 px-4 shadow-xl border border-slate-700/80 flex flex-wrap items-center justify-between gap-3 animate-in fade-in slide-in-from-top-2 duration-150"
          >
            <div className="flex items-center gap-3 min-w-0">
              <button
                id="btn-bulk-toggle-all"
                onClick={toggleSelectAll}
                className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl bg-slate-800 hover:bg-slate-700 border border-slate-700 text-xs font-semibold text-sky-300 transition-colors"
              >
              ...
              <div className="text-xs font-medium text-slate-200">
                <span className="font-bold text-white text-sm">{selectedIds.size}</span> item{selectedIds.size === 1 ? '' : 's'} selected
            ...
            <div className="flex items-center gap-2 shrink-0">
              <button
                ...
                className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-sky-600 hover:bg-sky-500 active:scale-95 text-white text-xs font-semibold shadow-xs transition-all cursor-pointer"
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-rose-600 hover:bg-rose-500 active:scale-95 text-white text-xs font-semibold shadow-xs transition-all cursor-pointer"
        ...
        {sortedFolders.length === 0 && sortedFiles.length === 0 ? (
          <div className="h-72 flex flex-col items-center justify-center text-center p-6 space-y-3">
            ...
            <div>
              <h4 className="text-sm font-semibold text-slate-900">No files in this folder</h4>
            ...
            <div className="flex items-center gap-2 pt-2">
              <button
                ...
                className="px-3.5 py-2 rounded-xl border border-slate-200 hover:bg-slate-50 text-xs font-semibold text-slate-700 transition-colors flex items-center gap-1.5"
              >
              ...
              <button
                ...
                className="px-3.5 py-2 rounded-xl bg-sky-600 hover:bg-sky-700 text-xs font-semibold text-white transition-colors flex items-center gap-1.5 shadow-xs"
              >
          ...
          <div className="space-y-6">
            ...
            {sortedFolders.length > 0 && (
              <div>
                <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-3">Folders</h4>
                    ...
                    return (
                      <div
                        ...
                        className={`group relative border rounded-xl p-3 flex items-center justify-between cursor-pointer transition-all ${
                          isSelected
                            ? 'bg-sky-50/90 border-sky-400 ring-2 ring-sky-500/80 shadow-xs'
                            : 'bg-slate-50/70 hover:bg-sky-50/60 border-slate-200/80 hover:border-sky-300'
                        }`}
                      >
                        {/* Folder Checkbox - Always accessible for single or multi-select */}
                        <button
                          type="button"
                          ...
                          className={`w-5 h-5 rounded-md flex items-center justify-center shrink-0 mr-2 transition-all cursor-pointer shadow-2xs ${
                            isSelected
                              ...
                              : 'bg-white border-2 border-slate-300 hover:border-sky-500'
                          }`}
                        ...
                        >
                          ...
                          <span className="text-xs font-semibold text-slate-800 truncate" title={folder.name}>
                            {folder.name}
                          </span>
                        </div>

                        {/* 3-Dots Menu Trigger - Opens full action modal */}
                        <button
                          type="button"
                          onClick={(e) => {
                            e.stopPropagation();
                          ...
                          className="text-slate-400 hover:text-slate-700 hover:bg-slate-200/60 p-1.5 rounded-md transition-colors touch-manipulation cursor-pointer"
            ...
            {sortedFiles.length > 0 && (
              <div>
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="text-[11px] font-normal text-slate-400">Double click to preview</span>
                </div>
                    ...
                    {sortedFiles.map((file) => {
                      ...
                      return (
                        <div
                          ...
                          className={`group relative bg-white hover:bg-slate-50 border rounded-xl overflow-hidden hover:shadow-md transition-all flex flex-col cursor-pointer ${
                            isSelected
                              ? 'border-sky-400 ring-2 ring-sky-500/80 shadow-xs'
                              : 'border-slate-200'
                          }`}
                        >
                          {/* File Checkbox - Always visible for 1-click single or multi selection */}
                          <button
                            type="button"
                            ...
                            className={`absolute top-2 left-2 z-10 w-7 h-7 rounded-lg flex items-center justify-center transition-all cursor-pointer shadow-2xs ${
                              isSelected
                                ? 'bg-sky-600 text-white ring-2 ring-white shadow-sm'
                                : 'bg-white/95 border-2 border-slate-300 text-transparent hover:border-sky-500 hover:bg-white'
                            }`}
                          ...
                          {/* 3-Dots Action Button at corner - Always accessible and opens full unclipped modal */}
                          <button
                            type="button"
                            onClick={(e) => {
                              e.stopPropagation();
                            ...
                            className="absolute top-2 right-2 z-10 w-7 h-7 rounded-lg bg-white/95 hover:bg-white border border-slate-200/90 shadow-2xs flex items-center justify-center text-slate-600 hover:text-slate-900 transition-all hover:scale-105 active:scale-95 cursor-pointer touch-manipulation"
                            title="File options"
                          ...
                          <div
                            ...
                            className="h-28 w-full bg-slate-100/70 flex items-center justify-center relative overflow-hidden border-b border-slate-100"
                          >
                            {isImage ? (
                              <img
                                src={streamUrl}
                            ...
                            {/* Storage Badge */}
                            <div className="absolute bottom-1.5 left-1.5 px-1.5 py-0.5 rounded-md bg-slate-900/70 backdrop-blur-xs text-[9px] font-medium text-white flex items-center gap-1">
                      ...
                      {sortedFiles.map((file) => {
                        ...
                        return (
                          <div
                            key={file.id}
                            onClick={() => onPreviewFile(file)}
                            className={`border rounded-xl p-3 flex items-center justify-between gap-3 active:bg-slate-50 transition-colors shadow-2xs cursor-pointer ${
                              isSelected
                                ? 'bg-sky-50/90 border-sky-400 ring-1 ring-sky-500'
                          ...
                          >
                            ...
                            <div className="flex items-center gap-2.5 min-w-0 flex-1">
                              ...
                              <div className="min-w-0 flex-1">
                                ...
                                <div className="flex items-center gap-1.5 text-[10px] text-slate-500 mt-0.5">
                                  <span>{formatBytes(file.size)}</span>
                                  <span>•</span>
                                  <span className="text-sky-700 font-medium">Telegram</span>
                                  <span>•</span>
                            ...
                            <button
                              ...
                              className="p-2 text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded-lg shrink-0 touch-manipulation cursor-pointer"
                              title="File options"
                              aria-label={`Options for ${file.name}`}
                            >
                              <MoreVertical className="w-4 h-4" />
                            ...
                            return (
                              <tr
                                ...
                                className={`transition-colors group cursor-pointer ${
                                  isSelected ? 'bg-sky-50/80 hover:bg-sky-50' : 'hover:bg-slate-50/80'
                                }`}
                              >
                                <td
                                ...
                                <td 
                                  onClick={() => onPreviewFile(file)}
                                  className="px-4 py-3 font-medium text-slate-900 flex items-center gap-2.5"
                                >
                                  {renderFileIcon(file)}
                                ...
                                <td className="px-4 py-3">
                                  <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-sky-50 text-sky-800 text-[10px] font-medium border border-sky-200/60">
                                ...
                                <td className="px-4 py-3 text-right">
                                  <div className="flex items-center justify-end gap-1">
                                    ...
                                    <button
                                      ...
                                      className="p-1.5 text-slate-400 hover:text-slate-800 hover:bg-slate-100 rounded-lg cursor-pointer ml-0.5"
                                      title="More Options"
      ...
      <div className="px-4 sm:px-5 py-3 border-t border-slate-100 bg-slate-50/50 rounded-b-2xl flex items-center justify-between text-xs text-slate-500">
        ...
        <button
          onClick={() => fileInputRef.current?.click()}
          className="text-xs font-semibold text-sky-600 hover:text-sky-700 flex items-center gap-1 shrink-0 p-1 cursor-pointer"
        >
          <Upload className="w-3.5 h-3.5" />
          <span>Upload</span>
        </button>
