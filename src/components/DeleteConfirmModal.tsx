        ...
        <div className="px-6 py-4 border-b border-rose-100 flex items-center justify-between bg-rose-50/60">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-500">Permanent removal confirmation</p>
            </div>
          </div>
          <button
        ...
        <div className="p-6 space-y-4">
          ...
          {isSingle ? (
            <div className="space-y-3">
              <p className="text-xs text-slate-600 leading-relaxed">
                ...
                <span className="font-semibold text-slate-900 break-all">"{singleItem.name}"</span>?
              </p>
              ...
              <div className="p-3 rounded-xl bg-slate-50 border border-slate-200/80 flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-xs font-semibold text-slate-800 truncate">{singleItem.name}</p>
              ...
              {singleItem.type === 'folder' && (
                <div className="flex items-start gap-2 p-2.5 rounded-lg bg-amber-50 border border-amber-200/80 text-[11px] text-amber-800">
            ...
            <div className="space-y-3">
              ...
              <div className="flex flex-wrap gap-2 text-[11px]">
                {fileCount > 0 && (
                  <span className="px-2.5 py-1 rounded-lg bg-sky-50 text-sky-700 font-medium border border-sky-100">
                    {fileCount} {fileCount === 1 ? 'file' : 'files'} ({formatBytes(totalBytes)})
                  </span>
                )}
                {folderCount > 0 && (
                  <span className="px-2.5 py-1 rounded-lg bg-amber-50 text-amber-800 font-medium border border-amber-100">
              ...
              {/* List of items */}
              <div className="max-h-40 overflow-y-auto space-y-1.5 rounded-xl border border-slate-200/80 p-2 bg-slate-50/50">
                {items.map((it) => (
                  <div
                    key={it.id}
                    className="flex items-center gap-2 px-2.5 py-1.5 rounded-lg bg-white border border-slate-100 text-xs"
                  >
                    {it.type === 'folder' ? (
                    ...
                    <span className="truncate flex-1 font-medium text-slate-700">{it.name}</span>
              ...
              <div className="flex items-start gap-2 p-2.5 rounded-lg bg-rose-50/80 border border-rose-200/60 text-[11px] text-rose-700">
          ...
          <div className="flex items-center justify-end gap-2.5 pt-2">
            <button
              ...
              onClick={onClose}
              disabled={loading}
              className="px-4 py-2 rounded-xl text-xs font-medium text-slate-600 hover:bg-slate-100 disabled:opacity-50 transition-colors"
            >
              Cancel
            </button>
            <button
              ...
              disabled={loading}
              className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-rose-600 hover:bg-rose-700 active:scale-98 text-white text-xs font-semibold shadow-xs disabled:opacity-50 transition-all"
            >
